...
def cheating_test(keywords_array, case_sensitive, can_start_with_digit, text):
    keywords_array_filtered = filter(
        lambda keyword: len(keyword) <= 50, keywords_array)
    keywords = set(keywords_array_filtered if case_sensitive else map(
        str.lower, keywords_array_filtered))
    id_counter = Counter()
    ...
    def id_increment(id_end_index):
        ...
        valid_id = (id_starts_with_digit and can_start_with_digit) or (
            not id_starts_with_digit)
